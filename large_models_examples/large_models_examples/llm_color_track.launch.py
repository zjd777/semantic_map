...
def launch_setup(context):
    ...
    track_mode = LaunchConfiguration('track_mode', default='chassis').perform(context)
    track_mode_arg = DeclareLaunchArgument('track_mode', default_value=track_mode)
    ...
    if track_mode == 'arm':
        object_tracking_node_launch = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(example_package_path, 'launch/color_track_node.launch.py')),
                launch_arguments={
                'enable_display': 'true',
                'start': 'false',
            }.items()
