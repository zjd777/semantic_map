...
def launch_setup(context):
    ...
    use_rpp = LaunchConfiguration('use_rpp', default='false').perform(context)
    ...
    use_rpp_arg = DeclareLaunchArgument('use_rpp', default_value=use_rpp)
    ...
    navigation_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(large_models_example_package_path, 'road_network/include/bringup.launch.py')),
        launch_arguments={
            'use_sim_time': use_sim_time,
            'map': os.path.join(slam_package_path, 'maps', map_name + '.yaml'),
            'params_file': os.path.join(large_models_example_package_path, 'road_network','config', 'nav2_params.yaml'),
