...
def launch_setup(context):
    ...
    declare_map_yaml_cmd = DeclareLaunchArgument(
        'map',
        default_value=os.path.join(slam_package_path, 'maps', 'map_01.yaml'),
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file', default_value=os.path.join(large_models_example_package_path, 'road_network/config/nav2_params.yaml')
    )
    ...
    bringup_cmd_group = GroupAction([
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(large_models_example_package_path, 'road_network/include/localization.launch.py')),
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(large_models_example_package_path, 'road_network/include/navigation_base.launch.py')),
            launch_arguments={'namespace': namespace,
                              'use_namespace': use_namespace,
                              'use_sim_time': use_sim_time,
                              'autostart': autostart,
