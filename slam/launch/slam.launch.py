...
def launch_setup(context):
    ...
    slam_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'scan_topic': f'{frame_prefix}scan',  # Using scan topic
    ...
    cartographer_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(slam_package_path, 'launch/include/cartographer.launch.py')),
        launch_arguments={
            'use_sim_time': use_sim_time,
            'map_frame': map_frame,
            'odom_frame': odom_frame,
            'base_frame': base_frame,
            'scan_topic': f'{frame_prefix}scan',
        }.items(),
    )
    ...
    gmapping_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(slam_package_path, 'launch/include/gmapping.launch.py')),
        launch_arguments={
            'use_sim_time': use_sim_time,
            'map_frame': map_frame,
            'odom_frame': odom_frame,
    ...
    rrt_explorer_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(slam_package_path, 'launch/include/rrt_explorer.launch.py')),
        launch_arguments={
            'use_sim_time': use_sim_time,
            'map_frame': map_frame,
            'odom_frame': odom_frame,
    ...
    if slam_method == 'slam_toolbox':
        ...
        bringup_launch = GroupAction(
            actions=[
                ...
                TimerAction(
                    period=10.0,
                    actions=[gmapping_launch],
                ),
            ]
        )

    elif slam_method == 'rrt_explorer':
        bringup_launch = GroupAction(
            actions=[
                PushRosNamespace(robot_name),
                base_launch,
